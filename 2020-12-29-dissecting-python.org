...
* Switching on lucid Python
  ...
  It was already established by *Python* and well-known as a
  /docstring/, the special sort of comments. Multi-line docstrings
  consist of a /summary line/, followed by a blank line, followed by a
  more elaborate description.

  #+BEGIN_SRC python :tangle tests/test_01_primal_testing_full_fledged.py
    # -*- coding: utf-8 -*-
    ...
    comment above with encoding definition. Docstrings do
    appear in the bytecode, so you can access this through the ``__doc__``
    attribute. This is also what you'll see if you call help() on a module
    ...
    readable only for humans, not compiled ever, and allows to the author
    write down all stuff necessary to be a foremost.
    ...
    nowadays most programs demand from reader such proficiency that it
    made them partially readable for humans and fully comprehended by the
    handful of true geeks only.
    ...
    like: “It works well enough to be scrutinized much more diligently.”
