...
* Testing Python primary constants
  ...
  *The essence* 
  ...
  Now you're checked up three _primary Python constants_ using
  =unittest= module and its three methods:

  - =assertTrue=
  - =assertFalse=
  - =assertIsNone=

  It is simple, minuscule and plain step onto the right path to cull
  knowledge.
...
* Switching on lucid Python
  ...
  manner, and *Python* would alleviate the pain in comprehension how
  the program works.

  As you might noticed we used the set of /inline comments/ to
  ...
  a /multi-line comments/ for large chunks of description texts. Yes,
  they are not obligation too, but in many cases they are
  indispensable as the source of clarity for author during writing the
  program and later for all its readers.

  It was already established by *Python* as a /docstring/, the special
  sort of comments. Multi-line docstrings consist of a /summary line/,
