...
* \alpha Dense Text
  ...
  of my professional skills: Leaning to Learn, Data Science, Model
  Thinking, Ovivo, Pythonanywhere, 0--key, oDesk, FastFresh,
  Py-Org-Cookbook, Perceptron in OOP Python3, Python Design Patterns
  Catalogue, StackOverflow.

  In 2017 I've begun to dissect an Artificial Intelligent solution for
...
* \mu{}otto
  ...
  $\displaystyle\sum_{n=1}^{team}\sum_{k=1}^{skill}\xRightarrow[\text{emacs}]{\text{org-mode}}\prod_{n=1}^{team}\sum_{k=1}^{skill}$
